...
/**
 * Test jeux de la vie ( conway ) Iteration 2 , dimenssion max 3x3
 * 
...
public class SimulatorTest {
	...
	public void testConwaySimulator1x1() {
		Simulator simulator = loadSimulator.loadSimulator("src/test/resources/iteration2/input-dim1.txt");
	...
	public void testConwaySimulator2x2v1() {
		Simulator simulator = loadSimulator.loadSimulator("src/test/resources/iteration2/input-dim2-v1.txt");
	...
	public void testConwaySimulator2x2v2() {
		Simulator simulator = loadSimulator.loadSimulator("src/test/resources/iteration2/input-dim2-v2.txt");
		for (int i = 0; i < simulator.getDimension(); i++) {
			for (int j = 0; j < simulator.getDimension(); j++)
				assertFalse(conwaySimulator.conwaySimulation(simulator.getDimension(), simulator.getCells())[i][j]
						.isAlive());
	...
	public void testConwaySimulator3x3v1() {
		Simulator simulator = loadSimulator.loadSimulator("src/test/resources/iteration2/input-dim3-v1.txt");
		Cell[][] CellsResult = { { new Cell(false), new Cell(true), new Cell(false) },
				{ new Cell(false), new Cell(true), new Cell(false) },
				{ new Cell(false), new Cell(true), new Cell(false) } };
		for (int i = 0; i < simulator.getDimension(); i++) {
			for (int j = 0; j < simulator.getDimension(); j++)
				assertTrue(CellsResult[i][j].equals(
						conwaySimulator.conwaySimulation(simulator.getDimension(), simulator.getCells())[i][j]));
		}
	}

	@Test
	public void testConwaySimulator3x3v2() {
		Simulator simulator = loadSimulator.loadSimulator("src/test/resources/iteration2/input-dim3-v2.txt");
