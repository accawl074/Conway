...
/**
 * Iteration2 max dimension 3, la régle :
...
public class ConwaySimulatorImpl implements IConwaySimulator {
			...
			for (int i = 0; i < dimension; i++) {
				for (int j = 0; j < dimension; j++) {
					if (cellsIn[i][j].isAlive()) {
						if (checkIfNNeighborsAlive(i, j, 2, cellsIn)
								|| checkIfNNeighborsAlive(i, j, 3, cellsIn)) {
							result[i][j] = new Cell(cellsIn[i][j].isAlive());
						} else {
							result[i][j] = new Cell(!cellsIn[i][j].isAlive());
						}

					} else {
						if (checkIfNNeighborsAlive(i, j, 3, cellsIn)) {
							result[i][j] = new Cell(!cellsIn[i][j].isAlive());
						} else {
							result[i][j] = new Cell(cellsIn[i][j].isAlive());
						}
	...
	/**
	 * Test pour une celule si il y a n cellules voisines vivantes
	 * 
	 * @param i
	 * @param j
	 * @param dimension
	 ...
	 * @return
	 */
	private static boolean checkIfNNeighborsAlive(int i, int j, int n, Cell[][] dataIn) {
		Cell[] neighborsCells = getNeighborsCells(i, j, dataIn);
		int counter = 0;
		for (int k = 0; k < 8; k++) {
			if (neighborsCells[k].isAlive()) {
				counter++;
			}
		}
	 ...
	 * @return
	 */
	private static Cell[] getNeighborsCells(int i, int j, Cell[][] cellsIn) {
